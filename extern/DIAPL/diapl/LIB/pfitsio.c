/*========================================================*/
/*                                                        */
/*  pfitsio.c       version 2.7.8         2006.10.25      */
/*                                                        */
                        ...
                        int *ncards, char ***header, int *n1)
{
  ...
  if (!(ldata=(double *)calloc((size_t)nx, sizeof(long))))
...
/*--------------------------------------------------------*/
double **read_FITS_2Dfile(char *iname, char datatype,
                         int *ncards, char ***header, int *n1, int *n2)
{
  ...
  if (get_FITS_key(hs, lhead, "NAXIS", value) == -1)
  {
    printf("\n\tERROR! %s: NAXIS not found in the header\n", iname);
    exit(EXIT_FAILURE);
  }
  sscanf(value, "%d", &naxes);
  if (naxes != 2)
  {
    printf("\n\tERROR! NAXIS= %d: %s is not 2 dimensional\n", naxes, iname);
    exit(EXIT_FAILURE);
  }

  if (get_FITS_key(hs, lhead, "NAXIS1", value) == -1)
  {
    printf("\n\tERROR! %s: NAXIS1 not found in header\n", iname);
    exit(EXIT_FAILURE);
  }
  sscanf(value, "%d", &nx);
  ...
  {
    printf("\n\tERROR! %s: NAXIS2 not found in header\n", iname);
    exit(EXIT_FAILURE);
  }
  sscanf(value, "%d", &ny);
  ...
  if (get_FITS_key(hs, lhead, "BITPIX", value) == -1)
  {
    printf("\n\tERROR! %s: BITPIX not found in header\n", iname);
    exit(EXIT_FAILURE);
  }
  sscanf(value, "%d", &bitpix);
  ...
  if (!(ldata=(double **)calloc((size_t)ny, sizeof(double *))))
    errmess("read_FITS_2Dfile(): calloc(ldata)");
  for (i=0; i<ny; i++)
  {
    if (!(ldata[i]=(double *)calloc((size_t)nx, sizeof(double))))
      errmess("read_FITS_2Dfile(): calloc(ldata[i])");
  }
  ...
  switch(bitpix)
  {
    case  16: if (datatype == 'u')
                for (i=0; i<ny; i++)
                  for (j=0; j<nx; j++)
                    ldata[i][j]=(double)((unsigned short *)buf)[i*nx+j];
              else
                for (i=0; i<ny; i++)
                  for (j=0; j<nx; j++)
                    ldata[i][j]=(double)((short *)buf)[i*nx+j];
              break;
    case  32: if (datatype == 'u')
                for (i=0; i<ny; i++)
                  for (j=0; j<nx; j++)
                    ldata[i][j]=(double)((unsigned *)buf)[i*nx+j];
              else
                for (i=0; i<ny; i++)
                  for (j=0; j<nx; j++)
                    ldata[i][j]=(double)((int *)buf)[i*nx+j];
              break;
    case  64: if (datatype == 'u')
                for (i=0; i<ny; i++)
                  for (j=0; j<nx; j++)
                    ldata[i][j]=(double)((unsigned long *)buf)[i*nx+j];
              else
                for (i=0; i<ny; i++)
                  for (j=0; j<nx; j++)
                    ldata[i][j]=(double)((long *)buf)[i*nx+j];
              break;
    case -32: for (i=0; i<ny; i++)
                for (j=0; j<nx; j++)
                  ldata[i][j]=((double *)buf)[i*nx+j];
              break;
    case -64: for (i=0; i<ny; i++)
                for (j=0; j<nx; j++)
                  ldata[i][j]=(double)((double *)buf)[i*nx+j];
              break;
    default:  printf("\n\tERROR! %s: bitpix=%d not supported\n", iname, bitpix);
              exit(EXIT_FAILURE);
  }
...
/*--------------------------------------------------------*/
void scale(int size, double *data, short *idata)
{
        int     i;
        double   min,
                max;
...
/*--------------------------------------------------------*/
void write_FITS_2Dimage(FILE *outf, int nx, int ny, int bytepix, void **data)
{
        char    *tmp;
        int     i;
  ...
  if (needswap())
  {
    for (i=0; i<ny; i++)
    {
      swap_bytes(data[i], (size_t)nx, bytepix);
      if (fwrite(data[i], (size_t)bytepix, (size_t)nx, outf) != (size_t) nx)
        errmess("write_FITS_2Dimage: fwrite(data[i])");
    }
  }
  else
    for (i=0; i<ny; i++)
  ...
  size=(size_t)nx*ny*bytepix;
  if ((ls=size%RECORD_SIZE) != 0) ls=RECORD_SIZE-ls;
  if (!(tmp=(char *)malloc(ls)))
    errmess("write_FITS_2Dimage(): malloc(tmp)");
  memset(tmp, 0, ls);
...
/*--------------------------------------------------------*/
void write_FITS_2Dfile(char *oname, int ncards, char **header, int nx, int ny,
                       int bytepix, void **data)
{
        FILE  *outf;
  ...
  write_FITS_2Dimage(outf, nx, ny, bytepix, data);
