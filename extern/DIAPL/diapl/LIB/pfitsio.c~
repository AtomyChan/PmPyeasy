                         ...
                         int *ncards, char ***header, int *n1, int *n2)
{
  ...
  if (!(ldata=(float **)calloc((size_t)ny, sizeof(float *))))
    errmess("read_FITS_2Dfile(): calloc(ldata)");
  for (i=0; i<ny; i++)
  {
    if (!(ldata[i]=(float *)calloc((size_t)nx, sizeof(float))))
  ...
  switch(bitpix)
  {
    ...
    case  64: if (datatype == 'u')
                for (i=0; i<ny; i++)
                  for (j=0; j<nx; j++)
                    ldata[i][j]=(float)((unsigned long *)buf)[i*nx+j];
              else
                for (i=0; i<ny; i++)
                  for (j=0; j<nx; j++)
                    ldata[i][j]=(float)((long *)buf)[i*nx+j];
              break;
