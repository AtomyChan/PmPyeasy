...
/*--------------------------------------------------------*/
double *trim_image(int *pnx, int *pny, double **data, int verbose)
{
        int   nx0, ny0,
              nx, ny,
              i, j,
  ...
  if (!(nldata=(double *)calloc(nx*ny, sizeof(double))))
...
int main(int argc, char *argv[])
{
        ...
        double   *cutim1,      /* template sector's buffer     */
...
/* get memory */
  ...
  if (!(tdata1=(double *)malloc(2*tsize*sizeof(double))))
    errmess("malloc(tdata1)");
  if (!(tdata2=(double *)malloc(2*tsize*sizeof(double))))
    errmess("malloc(tdata2)");

  if (!(corim8=(double *)malloc(2*corsize*sizeof(double))))
    errmess("malloc(corim8)");
  if (!(corim4=(double *)malloc(  corsize*sizeof(double))))
    errmess("malloc(corim4)");

  if (!(cutim1=(double *)malloc(csize*sizeof(double))))
    errmess("malloc(cutim1)");
  if (!(cutim2=(double *)malloc(csize*sizeof(double))))
    errmess("malloc(cutim2)");

  if (!(cdata1=(double *)malloc(2*csize*sizeof(double))))
    errmess("malloc(cdata1)");
  if (!(cdata2=(double *)malloc(2*csize*sizeof(double))))
...
/* get test image */
    ...
    if (par.verbose) printf("first guess: dx= %lf dy= %lf\n", x_shift, y_shift);
...
/* if no binning first guess is the final answer */
    ...
    if ((par.x_nbin != 1) || (par.y_nbin != 1))
    {
      ...
      if (par.verbose)
        printf("final guess: dx= %lf dy= %lf\n", x_shift, y_shift);
      ...
      if (par.verbose > 2)
      {
        ...
        fwrite(corim4, csize*sizeof(double), 1, outcf);
        fclose(outcf);
      }
    }
    else
