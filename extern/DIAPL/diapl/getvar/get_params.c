...
void get_params(char *parfname, char *instrname, PAR_STRUCT *par)
{
  ...
  for (i=0; !feof(inpf); i++)
  {
    ...
    else if (!strcasecmp(key, "NORMRAD"))   sscanf(val, "%lf", &par->normrad);
    ...
    else if (!strcasecmp(key, "NSIG_BKG"))  sscanf(val, "%lf", &par->nsig_bkg);
    else if (!strcasecmp(key, "BAD_VALUE")) sscanf(val, "%lf", &par->bad_value);
    else if (!strcasecmp(key, "ERR_CODE"))  sscanf(val, "%lf", &par->err);
    else if (!strcasecmp(key, "SMHW"))      sscanf(val, "%d", &par->smhw);
    else if (!strcasecmp(key, "MOHW"))      sscanf(val, "%d", &par->mohw);
    else if (!strcasecmp(key, "C_MIN"))     sscanf(val, "%lf", &par->c_min);
    else if (!strcasecmp(key, "NOBJ_INIT")) sscanf(val, "%d", &par->nobj_init);
    else if (!strcasecmp(key, "ID_RAD"))    sscanf(val, "%lf", &par->id_rad);
    else if (!strcasecmp(key, "NCONS_VAR1")) sscanf(val, "%d", &par->ncons_var1);
    else if (!strcasecmp(key, "NPTS_VAR2")) sscanf(val, "%d", &par->npts_var2);
    else if (!strcasecmp(key, "NSIG_VAR1")) sscanf(val, "%lf", &par->nsig_var1);
    else if (!strcasecmp(key, "NSIG_VAR2")) sscanf(val, "%lf", &par->nsig_var2);
    else if (!strcasecmp(key, "LIM_RATIO")) sscanf(val, "%lf", &par->lim_ratio);
    ...
    else if (!strcasecmp(key, "BAD_MARGIN")) sscanf(val, "%lf", &par->bad_margin);
    else if (!strcasecmp(key, "CENTER_HW")) sscanf(val, "%d", &par->center_hw);
    else if (!strcasecmp(key, "FILTER_HW")) sscanf(val, "%d", &par->filter_hw);
    else if (!strcasecmp(key, "FWHM_FRAC")) sscanf(val, "%lf", &par->fwhm_frac);
