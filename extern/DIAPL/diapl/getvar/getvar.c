...
/*--------------------------------------------------------*/
void writevar2fits(char *fname, int nx, int ny, double *data)
{
        char  **header;
        int   hsize, i;
  ...
  write_FITS_2D1file(fname, hsize, header, nx*ny, sizeof(double), data);
...
int main(int argc, char *argv[])
{
        ...
        int         nx0, ny0, k, nim, iim, hsize,
                    ...
                    nvar;
        double       x0_off, y0_off, x_tmp, y_tmp, fwhm_limit, *fwhm,
...
/*** get memory ***/
  ...
  if (!(fwhm  = (double *)malloc(nim*sizeof(double)))) errmess("malloc(fwhm)");
...
/*** get even more memory ***/
  ...
  if (!(im   =(double **)malloc(nim*sizeof(double *)))) errmess("malloc(im)");
  if (!(difim=(double **)malloc(nim*sizeof(double *)))) errmess("malloc(difim)");

  if (!(varim1=(double *)malloc(nx0*ny0*sizeof(double))))
    errmess("malloc(varim1)");
  if (!(varim2=(double *)malloc(nx0*ny0*sizeof(double))))
    errmess("malloc(varim2)");
  if (!(corrim=(double *)malloc(nx0*ny0*sizeof(double))))
...
/*** save variability images ***/
  if (!(tmpim=(double *)malloc(nx0*ny0*sizeof(double)))) errmess("malloc(tmpim)");

  memcpy(tmpim, varim1, nx0*ny0*sizeof(double));
  writevar2fits("var1.fits", nx0, ny0, tmpim);

  memcpy(tmpim, varim2, nx0*ny0*sizeof(double));
...
/*** get template photometry ***/
      ...
      x_tmp = (double)(x + x0_off);
      y_tmp = (double)(y + y0_off);
