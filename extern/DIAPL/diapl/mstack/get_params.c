...
void get_params(char *fname, char *instrname, PAR_STRUCT *par)
{
  ...
  while (!feof(inpf))
  {
    ...
    else if (!strcasecmp(key, "HIST_LOW"))    sscanf(val, "%lf", &par->low);
    else if (!strcasecmp(key, "HIST_HIGH"))   sscanf(val, "%lf", &par->high);
    ...
    else if (!strcasecmp(key, "THRESHOLD"))   sscanf(val, "%lf", &par->threshold);
    else if (!strcasecmp(key, "MIN_SCALE"))   sscanf(val, "%lf", &par->min_scale);
    else if (!strcasecmp(key, "MAX_SCALE"))   sscanf(val, "%lf", &par->max_scale);
