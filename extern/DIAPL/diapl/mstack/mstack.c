...
/*--------------------------------------------------------*/
double fcalc_median(int size, double *data)
{
        double median;
...
/*--------------------------------------------------------*/
double specstat(int n1, int n2, double *data, double *stddev)
{
        int     i,
                n;
        double  mean, lstddev;
  ...
  *stddev=(double)sqrt(lstddev-mean*mean);

  return((double)mean);
}
/*--------------------------------------------------------*/
void  stack_pix(int npix, int nim, double **data, double *bkg, double *scale,
                double refbg, double *imstack, PAR_STRUCT par)
{
  ...
  if (!(stat=(double *)calloc((size_t)nim, sizeof(double))))
...
/* get memory for image sections */
  npix=nx*ny;
  if (!(imstack=(double *)malloc(npix*sizeof(double))))
