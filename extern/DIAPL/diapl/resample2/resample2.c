...
void read_params(char *name, char *instrname, RINGPAR *par)
{
    ...
    for (i=0; !feof(inf); i++)
  {
    ...
    else if (!strcasecmp(key, "MAX_GRAD"))    sscanf(val, "%lf", &par->maxgrad);
    else if (!strcasecmp(key, "N_SIG_COSMICS")) sscanf(val, "%lf", &par->nsigcosmics);
    else if (!strcasecmp(key, "FIX_RINGS"))   sscanf(val, "%d", &par->fix);
    else if (!strcasecmp(key, "N_SIG_RINGS")) sscanf(val, "%lf", &par->nsig);
    else if (!strcasecmp(key, "N_SIG_RM"))    sscanf(val, "%lf", &par->nsigrm);
  ...
  for (i=0; !feof(inf); i++)
  {
    ...
    if (!strcasecmp(key, "END")) break;
    else if (!strcasecmp(key, "SAT_LEVEL")) sscanf(val, "%lf", &par->sat);
    else if (!strcasecmp(key, "MIN_LEVEL")) sscanf(val, "%lf", &par->min);
...
/* change bad column to background with noise (Igor Soszynski & Karol Zebrun) */
  ...
  if (!(im2=(double *)malloc(npix*sizeof(double)))) errmess("malloc(im2)");

/* the main procedure doing bicubic spline interpolations for all points */
  bicspl(im1, im2, nx, ny, coeffx, coeffy, ndeg, par);
  printf("ndeg=%d\n",ndeg);
  printf("=========cheek again the paramater===========\n");
  printf("===========im1,im2============\n");
  int j;
  for (j=0; j<10; j++) 
      printf("im1[%d]= %g  im2[%d]= %g\n",j,im1[j],j,im2[j]);
 free(im1);
     printf("nx=%d   ny=%d\n",nx,ny);
