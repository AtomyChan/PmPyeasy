...
void readpar(char *parfname, PARAMS *par)
{
  ...
  while (!feof(inpf))
  {
    ...
    else if (!strcasecmp(key, "SIGMA_F"))   sscanf(val, "%lf", &par->sigmaf);
...
/*--------------------------------------------------------*/
int readcoor(char *inpfname, double **x, double **y, double **zx, double **zy)
{
        char  buf[256];
        int   i;
        double *lx, *ly, *lzx, *lzy;
        FILE  *inpf;

  if (!(lx=(double *)calloc(1, sizeof(double))))
    errmess("readcoor: calloc(lx)");
  if (!(ly=(double *)calloc(1, sizeof(double))))
    errmess("readcoor: calloc(ly)");
  if (!(lzx=(double *)calloc(1, sizeof(double))))
    errmess("readcoor: calloc(lzx)");
  if (!(lzy=(double *)calloc(1, sizeof(double))))
  ...
  for (i=0; !feof(inpf); i++)
  {
    ...
    if (!(lzx=(double *)realloc(lzx, (i+1)*sizeof(double))))
      errmess("readcoor: realloc(lzx)");
    if (!(lzy=(double *)realloc(lzy, (i+1)*sizeof(double))))
      errmess("readcoor: realloc(lzy)");

    sscanf(buf, "%lf %lf %lf %lf", &lx[i], &ly[i], &lzx[i], &lzy[i]);
  }
...
/* make initial fit */
  ...
  if (!(nx=(double *)calloc(nobj, sizeof(double))))
    errmess("readcoor: calloc(nx)");
  if (!(ny=(double *)calloc(nobj, sizeof(double))))
    errmess("readcoor: calloc(ny)");
  if (!(nzx=(double *)calloc(nobj, sizeof(double))))
    errmess("readcoor: calloc(nzx)");
  if (!(nzy=(double *)calloc(nobj, sizeof(double))))
...
/* print results and store coefficients in binary file */
  ...
  printf("%s:  sigmax= %.4f   sigmay= %.4f   ndata= %d   nleft= %d\n",
   ...
   printf("to check what written into the file\n");
