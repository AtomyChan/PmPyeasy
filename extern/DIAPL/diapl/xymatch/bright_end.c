...
/*--------------------------------------------------------*/
int subselect(int nd, double *x, double *y, double *mag,
              double x1, double x2, double y1, double y2,
              double **tx, double **ty, double **tmag)
{
        int   i,
              ns;
        double *lx, *ly, *lmag;

  if (!(lx=(double *)calloc(nd, sizeof(double))))
    errmess("subselect(): calloc(lx)");
  if (!(ly=(double *)calloc(nd, sizeof(double))))
    errmess("subselect(): calloc(ly)");
  if (!(lmag=(double *)calloc(nd, sizeof(double))))
  ...
  if (!(lx=(double *)realloc(lx, ns*sizeof(double))))
    errmess("subselect(): realloc(lx)");
  if (!(ly=(double *)realloc(ly, ns*sizeof(double))))
    errmess("subselect(): realloc(ly)");
  if (!(lmag=(double *)realloc(lmag, ns*sizeof(double))))
...
/*--------------------------------------------------------*/
int bright_end(int nobj, double *x, double *y, double *mag,
                int nbright, double **xs, double **ys, short verbose)
{
        int   i, j, k,
              nmax,
              m,
...
/* check coordinates range */
  ...
  if (!(lxs=(double *)calloc(nbright, sizeof(double)))) errmess("calloc(lxs)");
  if (!(lys=(double *)calloc(nbright, sizeof(double)))) errmess("calloc(lys)");
