...
/*--------------------------------------------------------*/
double *read_FITS_1Dfile(char *iname, char datatype, int *hsize, char ***header,
                        int *n1)
{
        void    *buf;
        char    hline[72],
  ...
  switch(bitpix)
  {
    case   8: if (datatype == 'u')
                for (i=0; i<nx; i++)
                  ldata[i]=(double)((unsigned char *)buf)[i];
              else
                for (i=0; i<nx; i++)
                  ldata[i]=(double)((char *)buf)[i];
              break;
...
/*  bytepix=abs(bitpix)/8; */
  ...
  if (!(ldata=(double **)calloc(ny, sizeof(double *))))
    errmess("calloc(ldata)");
  for (i=0; i<ny; i++)
  {
    if (!(ldata[i]=(double *)calloc(nx, sizeof(double))))
  ...
  switch(bitpix)
  {
    case   8: if (datatype == 'u')
                for (i=0; i<ny; i++)
                  for (j=0; j<nx; j++)
                    ldata[i][j]=(double)((unsigned char *)buf)[i*nx+j];
              else
                for (i=0; i<ny; i++)
                  for (j=0; j<nx; j++)
                    ldata[i][j]=(double)((char *)buf)[i*nx+j];
              break;
    case  16: if (datatype == 'u')
                for (i=0; i<ny; i++)
                  for (j=0; j<nx; j++)
