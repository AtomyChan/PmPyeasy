...
class cosmicsimage:
	...
	def __str__(self):
		...
		if type(self.satstars) != type(None):
	...
	def guessbackgroundlevel(self):
		...
		"""
		if type(self.backgroundlevel) == type(None):
	...
	def subtractSkyLines(self, verbose=None):
		...
		if verbose:
			print "Subtracting sky lines using order of %i..."%self.skyOrder
	...
	def run(self, maxiter = 4, verbose = False):
		...
		elif self.satlevel > 0 and type(self.satstars) == type(None):
