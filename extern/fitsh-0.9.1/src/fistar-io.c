...
static int fprint_star_cx(FILE *fw,star *s,formatoutparam *fop)
{
 ...
 else			fprintf(fw,"%8.3f ",s->cand->cx+fop->bshx);
 return(0);
}
static int fprint_star_cy(FILE *fw,star *s,formatoutparam *fop)
{
 if ( s->cand==NULL )	fprintf(fw,"%8s ","-");
 else			fprintf(fw,"%8.3f ",s->cand->cy+fop->bshy);
 return(0);
}
...
{
 fprintf(fw,"%8.3f ",s->location.gcx+fop->bshx);
 return(0);
}
static int fprint_star_y(FILE *fw,star *s,formatoutparam *fop)
{
 fprintf(fw,"%8.3f ",s->location.gcy+fop->bshy);
 return(0);
}
...
{
 fprintf(fw,"%8.3f ",s->psf.gcx+fop->bshx);
 return(0);
}
static int fprint_star_py(FILE *fw,star *s,formatoutparam *fop)
{
 fprintf(fw,"%8.3f ",s->psf.gcy+fop->bshy);
 return(0);
}
...
static formatname formatnames[] =
{
 ...
 { I_DEVS,	 6, "S",	fprint_star_devs,	{ "s", NULL } },
 { I_DEVD,	 6, "D",	fprint_star_devd,	{ "d", NULL } },
 { I_DEVK,	 6, "K",	fprint_star_devk,	{ "k", NULL } },
...
int write_stars(FILE *fw,star *stars,int nstar,
	...
	double bshx,double bshy)
{
 char		*opnt,fbuff[8];
 star		*s;
 int		n,i,j,w,order,ndev,*of,f,ns;
 starshape	*sw;
 int		formatlookup[64];
 formatname	*wf;
 formatoutparam	fop;
