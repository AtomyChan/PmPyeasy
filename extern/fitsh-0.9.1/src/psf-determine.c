  ...
  {	for ( n=0 ; n<ncand ; n++ )
	 {	wc=&cands[n];
		for ( m=0 ; m<wc->nipoint ; m++ )
		 {	wi=&wc->ipoints[m];
			if ( mask != NULL && mask[wi->y][wi->x] )	continue;if ( mask != NULL && mask[wi->y][wi->x] )	continue;
  ...
  {	wc=&cands[n];
	...
	for ( m=0 ; m<wc->nipoint ; m++ )
	 {	wi=&wc->ipoints[m];
		...
		if ( x1<0.0 )	x1=0.0;if ( x1>wwd )	x1=wwd;
		if ( x2<0.0 )	x2=0.0;if ( x2>wwd )	x2=wwd;
		if ( y1<0.0 )	y1=0.0;if ( y1>wwd )	y1=wwd;
		if ( y2<0.0 )	y2=0.0;if ( y2>wwd )	y2=wwd;
  ...
  {	wc=&cands[n];
	...
	for ( m=0 ; m<wc->nipoint ; m++ )
	 {	wi=&wc->ipoints[m];
		...
		if ( x1<0.0 )	x1=0.0;if ( x1>fsize )	x1=fsize;
		if ( x2<0.0 )	x2=0.0;if ( x2>fsize )	x2=fsize;
		if ( y1<0.0 )	y1=0.0;if ( y1>fsize )	y1=fsize;
		if ( y2<0.0 )	y2=0.0;if ( y2>fsize )	y2=fsize;
	...
	double x0,double y0,double amp,psf *p,double mul)
{
 ...
 for ( i=0 ; i<nipoint ; i++ )
  {	x1=(double)(ipoints[i].x+0)-x0;
	...
	y1=afm*y1+afo;
	y2=afm*y2+afo;
	if ( x1<0.0 )	x1=0.0;if ( x1>=bx )	x1=bx;
	if ( x2<0.0 )	x2=0.0;if ( x2>=bx )	x2=bx;
	if ( y1<0.0 )	y1=0.0;if ( y1>=by )	y1=by;
	if ( y2<0.0 )	y2=0.0;if ( y2>=by )	y2=by;
	if ( x1<x2 && y1<y2 )
