...
/* this object has been obsoleted by `linkrange`, see linkpoint.[ch].	     */
...
/* llpoint_count(): functionality obsoleted by the new linkpoint.c library   */

/*
int llpoint_count(linkpoint **lparr,llpoint *llpoints,int nipoint)
{
 int	n,m,x,y,nx,ny;
 ...
 for ( n=0 ; n<nipoint ; n++ )
  {	x=llpoints[n].x;
	...
	ny=lparr[y][x].ny;
	for ( m=0 ; m<nipoint ; m++ )
	 {	if ( llpoints[m].x==nx && llpoints[m].y==ny )	break;	}
	if ( m<nipoint )	llpoints[n].nid=m;
	else			llpoints[n].nid=-1;
  }
