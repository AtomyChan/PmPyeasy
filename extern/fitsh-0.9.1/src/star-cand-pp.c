...
int search_star_candidates(fitsimage *img,char **mask,candidate **rcands,int *rncand,range *srcrange,double threshold,spatial *spbg,double skysigma)
{
 ...
 double		w,is,is2,ns,ia,ia2,na,sigs,siga;
 ...
 if ( imin<2 )	imin=2;if ( imax>=sy-2 )	imax=sy-3;
 if ( jmin<2 )	jmin=2;if ( jmax>=sx-2 )	jmax=sx-3;
 ...
 for ( i=imin ; i<=imax ; i++ )
  {	for ( j=jmin ; j<=jmax ; j++ )
	 {	double	sky;
		...
		if ( img->data[i-1][j-1]<w ) l++; if ( img->data[i-1][j+1]<w )	l++;
		k+=l*2;
