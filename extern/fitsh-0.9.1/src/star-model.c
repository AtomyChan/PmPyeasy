	 ...
	 {	dyda[0]=i0;		/* \partial_A	*/
		...
		if ( derivative_level>=1 )
	 	 {	cfdx[I00]=0.0;
			...
			cfdy[I04]=M03*s*cf[I03];
			cfdy[I50]=0.0;
			cfdx[I41]=M40*s*cf[I40];
			cfdx[I32]=M31*s*cf[I31];
			cfdx[I23]=M22*s*cf[I22];
			cfdx[I14]=M13*s*cf[I13];
			cfdx[I05]=M04*s*cf[I04];
			for ( i=0,w=0.0,k=0 ; i<=5 ; i++ )
			 {	for ( j=0 ; j<=i ; j++,k++ )
					w+=cfdy[k]*ix[i-j]*iy[j];
			 }
...
{
 int		i,j,k,sx,sy,n,asize,nipoint,ix0,ix1,iy0,iy1,isx,isy,ix,iy;
 int		p,q,r,t,c;
 ipoint  	*ipoints;
