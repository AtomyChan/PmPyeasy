...
void psf_2d_exact_xy(void *xpnt,double *a,double *yy,double *dyda,void *p)
{
 ...
 if ( px1<0.0 )	px1=0.0;if ( px1>maxv )	px1=maxv;
 if ( px2<0.0 )	px2=0.0;if ( px2>maxv )	px2=maxv;
 if ( py1<0.0 )	py1=0.0;if ( py1>maxv )	py1=maxv;
 if ( py2<0.0 )	py2=0.0;if ( py2>maxv )	py2=maxv;
	...
	starlocation *loc,psffit *pfp,psf *tpd)
{
 ...
 int	i,j,k;
 double	***btarr,**coeff,*cpoly,afp[4],lam,w,sum,ox,oy,scale,gg;
...
int fit_star_psf_native(fitsimage *img,char **mask,candidate *cands,int ncand,star **rstars,int *rnstar,psffit *pfp,psf *tpd)
{
 star		*stars,*ws;
 candidate	*wc;
 int		nstar,n,ix,iy,sx,sy,bsize;
 int		hsize,order,grid,i;
 void		**fitpnt;
 double		*yvals;
 starlocation	loc;
 ipoint		*ipoints;
