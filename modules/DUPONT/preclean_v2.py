...
for fl in SimSpec.glob.glob(SimSpec.WorkDir+'*_SQ???.fits'):
    ...
    if y>SimSpec.param['cropheight']:
        ...
        tsec='[*,*]'
    SimSpec.iraf.ccdproc(fl, output="", ccdtype="",  fixpix='no', oversca='yes', trim='yes', zerocor='no', darkcor='no', flatcor='no', biassec="image", readaxis="column", trimsec=tsec, zero='bias', flat='nflat')
    import time
    print ('Waiting a bit to allow the buggy IRAF to settle down.')
    time.sleep(1)
